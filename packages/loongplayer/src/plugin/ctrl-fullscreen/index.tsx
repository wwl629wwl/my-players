...
export class FullScreen extends UIPlugin {
    ...
    protected render(): JSX.Element | string | HTMLElement {
        ...
        return (
            <div class="loplayer-controller-middle-item loplayer-controller-fullscreen-container" onClick={() => handleClick()} >
                <div innerHTML={state.isEnterFullscreen ? leaveFullscreen : fullscreen} style={{ width: '100%', height: '100%' }}></div>
                <span class="loplayer-controller-middle-item-tip">{state.isEnterFullscreen ? '退出全屏' : '全屏'}</span>
            </div>
        )
    }
