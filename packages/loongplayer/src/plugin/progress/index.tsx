...
export class Progress extends UIPlugin {
    ...
    // createsignal 返回两个 一个是 state 一个是 function 然后使用也是使用()的方式
    ...
    // 这是一个 TimeRanges 对象，表示已经缓冲的数据区间。它是一个只读属性，不能修改。
    ...
    get thumbLeft() {
        ...
        const minPer = 2;
        const maxPer = 98 - this.thumbContainer.clientWidth / 2 / this.player.nodes.controllerBarMiddle.clientWidth * 100;
    ...
    protected render(): JSX.Element | string | HTMLElement {
        ...
        return (
            <div class="loplayer-progress-container" style={{ cursor: 'pointer', position: 'absolute', bottom: '0', width: '100%' }} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
                <div class="loplayer-progress-thumbnail-container" style={{
                    left: `${this.thumbLeft}%`,
                    display: this.player.rootStore.actionStore.state.isHoverProgress ? '' : 'none'
                }} ref={this.thumbContainer}>
                    <div class="loplayer-progress-thumbnail" style={{
                        display: this.player.config.thumbnail ? '' : 'none',
                    }}>
                        <img src={this.imageUrl} alt="预览图" />
                    </div>
                    <div class="loplayer-progress-time-label">{this.timeLabel}</div>
                </div>
                <Slider
